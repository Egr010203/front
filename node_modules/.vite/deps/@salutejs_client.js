import { i as __toCommonJS, n as __esmMin, r as __exportAll } from "./chunk-_TIqcEvS.js";
//#region node_modules/@salutejs/client/esm/common-ba25e019.js
function __rest(s, e) {
	var t = {};
...
var __assign, createNanoEvents, createNanoObservable;
var init_common_ba25e019 = __esmMin((() => {
	__assign = function() {
	...
	createNanoEvents = function() {
		...
		var emit = function(event) {
			...
			for (var _a = 0, _b = events[event] || []; _a < _b.length; _a++) _b[_a].apply(void 0, args);
		};
	...
	createNanoObservable = function(observerFunc) {
		var _a = createNanoEvents(), on = _a.on, emit = _a.emit;
		var subscribe = function(_a) {
			var next = _a.next;
			return { unsubscribe: on("next", next) };
		};
		observerFunc({ next: function(data) {
			emit("next", data);
		} });
		return { subscribe };
	};
...
//#endregion
//#region node_modules/@salutejs/client/esm/createAssistant.js
function rng() {
	if (!getRandomValues) {
...
var getRandomValues, rnds8, REGEX, byteToHex, i, isDeepEqual, findCommandIndex, appInitialData, excludeTags, postMessage_1, historyBack_1, createAssistant;
var init_createAssistant = __esmMin((() => {
	init_common_ba25e019();
	...
	findCommandIndex = function(arr, command) {
		var insets = [
			"insets",
		...
		var index = -1;
		if (command.type === "character") index = arr.findIndex(function(c) {
			return c.type === "character" && c.character.id === command.character.id;
		});
		else if (insets.includes(command.type)) index = arr.findIndex(function(c) {
			return c.type === command.type;
		});
		else if (command.type === "app_context") index = arr.findIndex(function(c) {
			return c.type === "app_context";
		});
		else index = arr.findIndex(function(c) {
			return isDeepEqual(c, command);
		});
		return index;
	};
	appInitialData = (function() {
		var isPulled = false;
		var pulled = [];
		var committed = [];
		var diff = [];
		...
		return {
			...
			*/
			isCommitted: function(command) {
				...
				if (isCommitted) committed.splice(commandIndex, 1);
				return isCommitted;
			},
			/**
			...
			*/
			find: function(_a) {
				...
				var result = __spreadArray([], window.appInitialData || []).find(function(initialCommand) {
					if (!command && type && type === initialCommand.type) return true;
					if (!(command && "smart_app_data" in initialCommand)) return;
					if (command === initialCommand.smart_app_data.command || command === initialCommand.smart_app_data.type) return true;
					return false;
				});
	...
	if (/[a-zA-Z]/.test("1.37.1")) console.info("%cPlease use the latest version of SaluteJS Client. Your version is 1.37.1", "color: yellow; font-size: 14px");
	if (typeof window !== "undefined" && inIframe()) {
		postMessage_1 = function(action) {
			var _a;
			(_a = window.parent) === null || _a === void 0 || _a.postMessage(JSON.stringify(action), "*");
		};
		historyBack_1 = function() {
			var prevPage = window.location.href;
			window.history.back();
			setTimeout(function() {
				if (window.location.href === prevPage) postMessage_1({ type: "close" });
			}, 500);
		};
				...
				if (typeof e.data === "string") {
					...
					switch (data.type) {
						case "onBack":
							...
							break;
						case "onData":
							(_b = (_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onData) === null || _b === void 0 || _b.call(_a, data.payload);
							break;
						case "onRequestState":
							postMessage_1({
								type: "state",
								payload: (_d = (_c = window.AssistantClient) === null || _c === void 0 ? void 0 : _c.onRequestState) === null || _d === void 0 ? void 0 : _d.call(_c),
						...
						case "onRequestRecoveryState":
							postMessage_1({
								type: "recoveryState",
								payload: (_f = (_e = window.AssistantClient) === null || _e === void 0 ? void 0 : _e.onRequestRecoveryState) === null || _f === void 0 ? void 0 : _f.call(_e)
							});
							break;
						case "onStart":
							(_h = (_g = window.AssistantClient) === null || _g === void 0 ? void 0 : _g.onStart) === null || _h === void 0 || _h.call(_g);
							break;
						default:
		...
		window.addEventListener("keydown", function(_a) {
			...
			switch (_a.code) {
				case "Enter":
					if (document.activeElement && !excludeTags.includes(document.activeElement.tagName)) (_c = (_b = document.activeElement).click) === null || _c === void 0 || _c.call(_b);
					break;
				case "Escape":
					historyBack_1();
					break;
	...
	createAssistant = function(_a) {
		...
		var emitCommand = function(command) {
			if (command.type === "smart_app_data") emitOriginal("command", command.smart_app_data);
			if (command.type === "smart_app_error") emitOriginal("error", command.smart_app_error);
		...
		var cancelTts = typeof ((_b = window.AssistantHost) === null || _b === void 0 ? void 0 : _b.cancelTts) !== "undefined" ? function() {
			var _a, _b;
			(_b = (_a = window.AssistantHost).cancelTts) === null || _b === void 0 || _b.call(_a, "");
		} : void 0;
		...
		var saveFirstSmartAppDataMid = function(mid) {
			if (typeof window.__ASSISTANT_CLIENT__.firstSmartAppDataMid === "undefined") window.__ASSISTANT_CLIENT__.firstSmartAppDataMid = mid;
		};
		window.AssistantClient = {
			onData: function(command) {
				var _a, _b, _c, _d, _e;
				if (appInitialData.isCommitted(command)) return;
				emitAllCommands(command.type, command);
				if (command.type === "smart_app_data" && (((_a = command.sdk_meta) === null || _a === void 0 ? void 0 : _a.mid) || "-1") !== "-1") saveFirstSmartAppDataMid((_b = command.sdk_meta) === null || _b === void 0 ? void 0 : _b.mid);
				if (command.type === "system" && ((_d = (_c = command.system) === null || _c === void 0 ? void 0 : _c.command) === null || _d === void 0 ? void 0 : _d.toUpperCase()) === "BACK") return;
				if (command.type === "tts_state_update") emitOriginal("tts", {
					state: command.state,
					owner: command.owner
				});
				if ((command.type === "smart_app_data" || command.type === "smart_app_error") && ((_e = command.sdk_meta) === null || _e === void 0 ? void 0 : _e.requestId) && observables.has(command.sdk_meta.requestId)) {
					...
					if (Object.keys(meta).length > 0 || requestId) {
						...
						if (requestId) command.sdk_meta = { requestId };
					}
					next === null || next === void 0 || next(command.type === "smart_app_data" ? command : command);
				}
				emitCommand(command);
			},
			...
			onRequestRecoveryState: function() {
				if (currentGetRecoveryState) return currentGetRecoveryState();
			},
			onStart: function() {
				emitOriginal("start");
				emitAppInitialData();
		...
		var readyFn = function() {
			return __awaiter(void 0, void 0, void 0, function() {
				...
				return __generator(this, function(_d) {
					readyRetries += 1;
					if (typeof ((_a = window.AssistantHost) === null || _a === void 0 ? void 0 : _a.ready) !== "function") return [2, new Promise(function(resolve, reject) {
						var _a;
						if (readyRetries > 3) throw new Error("window.AssistantHost is not ready. The ready method has the type \"" + typeof ((_a = window.AssistantHost) === null || _a === void 0 ? void 0 : _a.ready) + "\"");
					...
					if (firstSmartAppDataMid !== "-1") saveFirstSmartAppDataMid(firstSmartAppDataMid);
		...
		if (ready) window.setTimeout(readyFn);
		var sendData = function(_a, onData) {
			...
			if ((_b = window.AssistantHost) === null || _b === void 0 ? void 0 : _b.sendDataContainer) {
				if (onData == null) {
					(_c = window.AssistantHost) === null || _c === void 0 || _c.sendDataContainer(JSON.stringify({
						data: action,
						message_name: name || "",
						requestId,
						mode
				...
				if (requestId && observables.has(requestId)) throw new Error("requestId должен быть уникальным");
				var realRequestId_1 = requestId || v4();
				var subscribe = createNanoObservable(function(_a) {
					var _b;
					var next = _a.next;
					(_b = window.AssistantHost) === null || _b === void 0 || _b.sendDataContainer(JSON.stringify({
						data: action,
						message_name: name || "",
			...
			if (onData != null) throw new Error("Не поддерживается в данной версии клиента");
			(_d = window.AssistantHost) === null || _d === void 0 || _d.sendData(JSON.stringify(action), name || null);
			return function() {};
		};
		var sendAction = function(action, onData, onError, params) {
			if (params === void 0) params = {};
			return sendData(__assign(__assign({}, params), { action }), function(data) {
				if (data.type === "smart_app_data") onData === null || onData === void 0 || onData(data.smart_app_data);
				if (data.type === "smart_app_error") onError === null || onError === void 0 || onError(data.smart_app_error);
			});
		};
		return {
			cancelTts,
			...
			getGeo: typeof ((_c = window.AssistantHost) === null || _c === void 0 ? void 0 : _c.getGeo) !== "undefined" ? function() {
				var _a, _b;
				(_b = (_a = window.AssistantHost).getGeo) === null || _b === void 0 || _b.call(_a);
			} : void 0,
			...
			setSuggests: function(suggestions) {
				var _a;
				(_a = window.AssistantHost) === null || _a === void 0 || _a.setSuggests(JSON.stringify({ suggestions: { buttons: suggestions } }));
			},
			setHints: function(hints) {
				var _a;
				(_a = window.AssistantHost) === null || _a === void 0 || _a.setHints(JSON.stringify({ hints }));
			},
			sendText: function(message) {
				var _a;
			...
			setHeaderButtons: function(headerButtons) {
				var _a, _b;
				if (!((_a = window.AssistantHost) === null || _a === void 0 ? void 0 : _a.setHeaderButtons)) throw new Error("setHeaderButtons не поддерживается в данной версии клиента");
				(_b = window.AssistantHost) === null || _b === void 0 || _b.setHeaderButtons(JSON.stringify(headerButtons));
			},
			ready: readyFn
		};
	};
	if (typeof window !== "undefined") window.__ASSISTANT_CLIENT__ = { version: "1.37.1" };
}));
//#endregion
//#region node_modules/@salutejs/client/esm/sdk-7c3cb15e.js
...
var commonjsGlobal, proto, indexMinimal, minimal$1, aspromise, hasRequiredAspromise, base64, hasRequiredBase64, eventemitter, hasRequiredEventemitter, float, hasRequiredFloat, inquire_1, hasRequiredInquire, utf8, hasRequiredUtf8, pool_1, hasRequiredPool, longbits, hasRequiredLongbits, hasRequiredMinimal$1, writer, hasRequiredWriter, writer_buffer, hasRequiredWriter_buffer, reader, hasRequiredReader, reader_buffer, hasRequiredReader_buffer, rpc, service, hasRequiredService, hasRequiredRpc, roots, hasRequiredRoots, hasRequiredIndexMinimal, minimal, hasRequiredMinimal, protoExports, VpsVersion, MessageNames, createClientMethods, safeJSONParse, compileBasePayload, appendHeader, removeHeader, createProtocol;
var init_sdk_7c3cb15e = __esmMin((() => {
	init_common_ba25e019();
	...
	(function(module) {
		(function(global, factory) {
			if (typeof commonjsRequire === "function" && module && module.exports) module.exports = factory(requireMinimal());
			...
			$root.Message = (function() {
				...
				*/
				Message.encode = function encode(message, writer) {
					...
					if (message.messageId != null && Object.hasOwnProperty.call(message, "messageId")) writer.uint32(16).int64(message.messageId);
					if (message.last != null && Object.hasOwnProperty.call(message, "last")) writer.uint32(24).int32(message.last);
					if (message.token != null && Object.hasOwnProperty.call(message, "token")) writer.uint32(34).string(message.token);
					if (message.voice != null && Object.hasOwnProperty.call(message, "voice")) $root.Voice.encode(message.voice, writer.uint32(42).fork()).ldelim();
					if (message.text != null && Object.hasOwnProperty.call(message, "text")) $root.Text.encode(message.text, writer.uint32(50).fork()).ldelim();
					if (message.systemMessage != null && Object.hasOwnProperty.call(message, "systemMessage")) $root.SystemMessage.encode(message.systemMessage, writer.uint32(58).fork()).ldelim();
					if (message.legacyDevice != null && Object.hasOwnProperty.call(message, "legacyDevice")) $root.LegacyDevice.encode(message.legacyDevice, writer.uint32(66).fork()).ldelim();
					if (message.settings != null && Object.hasOwnProperty.call(message, "settings")) $root.Settings.encode(message.settings, writer.uint32(74).fork()).ldelim();
					if (message.status != null && Object.hasOwnProperty.call(message, "status")) $root.Status.encode(message.status, writer.uint32(82).fork()).ldelim();
					if (message.userChannel != null && Object.hasOwnProperty.call(message, "userChannel")) writer.uint32(90).string(message.userChannel);
					if (message.vpsToken != null && Object.hasOwnProperty.call(message, "vpsToken")) writer.uint32(98).string(message.vpsToken);
					if (message.devContext != null && message.devContext.length) for (var i = 0; i < message.devContext.length; ++i) $root.DevContext.encode(message.devContext[i], writer.uint32(106).fork()).ldelim();
					if (message.messageName != null && Object.hasOwnProperty.call(message, "messageName")) writer.uint32(114).string(message.messageName);
					if (message.version != null && Object.hasOwnProperty.call(message, "version")) writer.uint32(120).int32(message.version);
					if (message.device != null && Object.hasOwnProperty.call(message, "device")) $root.Device.encode(message.device, writer.uint32(130).fork()).ldelim();
					if (message.bytes != null && Object.hasOwnProperty.call(message, "bytes")) $root.Bytes.encode(message.bytes, writer.uint32(138).fork()).ldelim();
					if (message.initialSettings != null && Object.hasOwnProperty.call(message, "initialSettings")) $root.InitialSettings.encode(message.initialSettings, writer.uint32(146).fork()).ldelim();
					if (message.timestamp != null && Object.hasOwnProperty.call(message, "timestamp")) writer.uint32(152).int64(message.timestamp);
					if (message.meta != null && Object.hasOwnProperty.call(message, "meta")) for (var keys = Object.keys(message.meta), i = 0; i < keys.length; ++i) writer.uint32(162).fork().uint32(10).string(keys[i]).uint32(18).string(message.meta[keys[i]]).ldelim();
					if (message.cancel != null && Object.hasOwnProperty.call(message, "cancel")) $root.Cancel.encode(message.cancel, writer.uint32(170).fork()).ldelim();
					if (message.getHistoryRequest != null && Object.hasOwnProperty.call(message, "getHistoryRequest")) $root.GetHistoryRequest.encode(message.getHistoryRequest, writer.uint32(178).fork()).ldelim();
					if (message.mute != null && Object.hasOwnProperty.call(message, "mute")) $root.Mute.encode(message.mute, writer.uint32(186).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				Message.decode = function decode(reader, length) {
					...
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Message(), key, value;
			...
			$root.InitialSettings = (function() {
				...
				*/
				InitialSettings.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.userId != null && Object.hasOwnProperty.call(message, "userId")) writer.uint32(10).string(message.userId);
					if (message.userChannel != null && Object.hasOwnProperty.call(message, "userChannel")) writer.uint32(18).string(message.userChannel);
					if (message.device != null && Object.hasOwnProperty.call(message, "device")) $root.Device.encode(message.device, writer.uint32(26).fork()).ldelim();
					if (message.settings != null && Object.hasOwnProperty.call(message, "settings")) $root.Settings.encode(message.settings, writer.uint32(34).fork()).ldelim();
					if (message.locale != null && Object.hasOwnProperty.call(message, "locale")) writer.uint32(42).string(message.locale);
					return writer;
				};
				/**
				...
				*/
				InitialSettings.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.InitialSettings();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Device = (function() {
				...
				*/
				Device.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.platformType != null && Object.hasOwnProperty.call(message, "platformType")) writer.uint32(10).string(message.platformType);
					if (message.platformVersion != null && Object.hasOwnProperty.call(message, "platformVersion")) writer.uint32(18).string(message.platformVersion);
					if (message.surface != null && Object.hasOwnProperty.call(message, "surface")) writer.uint32(26).string(message.surface);
					if (message.surfaceVersion != null && Object.hasOwnProperty.call(message, "surfaceVersion")) writer.uint32(34).string(message.surfaceVersion);
					if (message.features != null && Object.hasOwnProperty.call(message, "features")) writer.uint32(42).string(message.features);
					if (message.capabilities != null && Object.hasOwnProperty.call(message, "capabilities")) writer.uint32(50).string(message.capabilities);
					if (message.deviceId != null && Object.hasOwnProperty.call(message, "deviceId")) writer.uint32(58).string(message.deviceId);
					if (message.deviceManufacturer != null && Object.hasOwnProperty.call(message, "deviceManufacturer")) writer.uint32(66).string(message.deviceManufacturer);
					if (message.deviceModel != null && Object.hasOwnProperty.call(message, "deviceModel")) writer.uint32(74).string(message.deviceModel);
					if (message.additionalInfo != null && Object.hasOwnProperty.call(message, "additionalInfo")) writer.uint32(82).string(message.additionalInfo);
					if (message.tenant != null && Object.hasOwnProperty.call(message, "tenant")) writer.uint32(90).string(message.tenant);
					return writer;
				};
				/**
				...
				*/
				Device.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Device();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Settings = (function() {
				...
				*/
				Settings.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.dubbing != null && Object.hasOwnProperty.call(message, "dubbing")) writer.uint32(8).int32(message.dubbing);
					if (message.echo != null && Object.hasOwnProperty.call(message, "echo")) writer.uint32(16).int32(message.echo);
					if (message.ttsEngine != null && Object.hasOwnProperty.call(message, "ttsEngine")) writer.uint32(26).string(message.ttsEngine);
					if (message.asrEngine != null && Object.hasOwnProperty.call(message, "asrEngine")) writer.uint32(34).string(message.asrEngine);
					if (message.asrAutoStop != null && Object.hasOwnProperty.call(message, "asrAutoStop")) writer.uint32(40).int32(message.asrAutoStop);
					if (message.devMode != null && Object.hasOwnProperty.call(message, "devMode")) writer.uint32(48).int32(message.devMode);
					if (message.authConnector != null && Object.hasOwnProperty.call(message, "authConnector")) writer.uint32(58).string(message.authConnector);
					if (message.surface != null && Object.hasOwnProperty.call(message, "surface")) writer.uint32(66).string(message.surface);
					return writer;
				};
				/**
				...
				*/
				Settings.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Settings();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.LegacyDevice = (function() {
				...
				*/
				LegacyDevice.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.clientType != null && Object.hasOwnProperty.call(message, "clientType")) writer.uint32(10).string(message.clientType);
					if (message.channel != null && Object.hasOwnProperty.call(message, "channel")) writer.uint32(18).string(message.channel);
					if (message.channelVersion != null && Object.hasOwnProperty.call(message, "channelVersion")) writer.uint32(26).string(message.channelVersion);
					if (message.platformName != null && Object.hasOwnProperty.call(message, "platformName")) writer.uint32(34).string(message.platformName);
					if (message.platformVersion != null && Object.hasOwnProperty.call(message, "platformVersion")) writer.uint32(42).string(message.platformVersion);
					if (message.sdkVersion != null && Object.hasOwnProperty.call(message, "sdkVersion")) writer.uint32(50).string(message.sdkVersion);
					if (message.protocolVersion != null && Object.hasOwnProperty.call(message, "protocolVersion")) writer.uint32(58).string(message.protocolVersion);
					return writer;
				};
				/**
				...
				*/
				LegacyDevice.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.LegacyDevice();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Voice = (function() {
				...
				*/
				Voice.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Voice();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Text = (function() {
				...
				*/
				Text.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Text();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.SystemMessage = (function() {
				...
				*/
				SystemMessage.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.SystemMessage();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Status = (function() {
				...
				*/
				Status.encode = function encode(message, writer) {
					...
					if (message.technicalDescription != null && Object.hasOwnProperty.call(message, "technicalDescription")) writer.uint32(26).string(message.technicalDescription);
					return writer;
				};
				/**
				...
				*/
				Status.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Status();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Bytes = (function() {
				...
				*/
				Bytes.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.data != null && Object.hasOwnProperty.call(message, "data")) writer.uint32(10).bytes(message.data);
					if (message.desc != null && Object.hasOwnProperty.call(message, "desc")) writer.uint32(18).string(message.desc);
					return writer;
				};
				/**
				...
				*/
				Bytes.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Bytes();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.DevContext = (function() {
				...
				*/
				DevContext.encode = function encode(message, writer) {
					...
					if (message.timestampMs != null && Object.hasOwnProperty.call(message, "timestampMs")) writer.uint32(16).int64(message.timestampMs);
					if (message.data != null && Object.hasOwnProperty.call(message, "data")) writer.uint32(26).string(message.data);
					return writer;
				};
				/**
				...
				*/
				DevContext.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.DevContext();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Cancel = (function() {
				...
				*/
				Cancel.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Cancel();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							default:
			...
			$root.Mute = (function() {
				...
				*/
				Mute.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Mute();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							default:
			...
			$root.GetHistoryRequest = (function() {
				...
				*/
				GetHistoryRequest.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.messageTypes != null && message.messageTypes.length) for (var i = 0; i < message.messageTypes.length; ++i) writer.uint32(10).string(message.messageTypes[i]);
					if (message.app != null && Object.hasOwnProperty.call(message, "app")) $root.App.encode(message.app, writer.uint32(18).fork()).ldelim();
					if (message.offset != null && Object.hasOwnProperty.call(message, "offset")) $root.Offset.encode(message.offset, writer.uint32(26).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				GetHistoryRequest.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.GetHistoryRequest();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.App = (function() {
				...
				*/
				App.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.type != null && Object.hasOwnProperty.call(message, "type")) $root.google.protobuf.StringValue.encode(message.type, writer.uint32(10).fork()).ldelim();
					if (message.projectId != null && Object.hasOwnProperty.call(message, "projectId")) $root.google.protobuf.StringValue.encode(message.projectId, writer.uint32(18).fork()).ldelim();
					if (message.systemName != null && Object.hasOwnProperty.call(message, "systemName")) $root.google.protobuf.StringValue.encode(message.systemName, writer.uint32(26).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				App.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.App();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Offset = (function() {
				...
				*/
				Offset.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.limit != null && Object.hasOwnProperty.call(message, "limit")) $root.google.protobuf.StringValue.encode(message.limit, writer.uint32(10).fork()).ldelim();
					if (message.contentId != null && Object.hasOwnProperty.call(message, "contentId")) $root.google.protobuf.StringValue.encode(message.contentId, writer.uint32(18).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				Offset.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Offset();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.ChatHistoryRequest = (function() {
				...
				*/
				ChatHistoryRequest.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.uuid != null && Object.hasOwnProperty.call(message, "uuid")) $root.Uuid.encode(message.uuid, writer.uint32(10).fork()).ldelim();
					if (message.device != null && Object.hasOwnProperty.call(message, "device")) $root.Device.encode(message.device, writer.uint32(18).fork()).ldelim();
					if (message.getHistoryRequest != null && Object.hasOwnProperty.call(message, "getHistoryRequest")) $root.GetHistoryRequest.encode(message.getHistoryRequest, writer.uint32(34).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				ChatHistoryRequest.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.ChatHistoryRequest();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Uuid = (function() {
				...
				*/
				Uuid.encode = function encode(message, writer) {
					...
					if (message.userId != null && Object.hasOwnProperty.call(message, "userId")) writer.uint32(10).string(message.userId);
					if (message.userChannel != null && Object.hasOwnProperty.call(message, "userChannel")) writer.uint32(18).string(message.userChannel);
					if (message.sub != null && Object.hasOwnProperty.call(message, "sub")) writer.uint32(26).string(message.sub);
					return writer;
				};
				/**
				...
				*/
				Uuid.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Uuid();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.GetHistoryResponse = (function() {
				...
				*/
				GetHistoryResponse.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.historyMessages != null && message.historyMessages.length) for (var i = 0; i < message.historyMessages.length; ++i) $root.HistoryMessages.encode(message.historyMessages[i], writer.uint32(10).fork()).ldelim();
					return writer;
				};
				/**
				...
				*/
				GetHistoryResponse.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.GetHistoryResponse();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.HistoryMessages = (function() {
				...
				*/
				HistoryMessages.encode = function encode(message, writer) {
					if (!writer) writer = $Writer.create();
					if (message.content != null && Object.hasOwnProperty.call(message, "content")) writer.uint32(10).string(message.content);
					if (message.contentId != null && Object.hasOwnProperty.call(message, "contentId")) writer.uint32(18).string(message.contentId);
					if (message.timeCreated != null && Object.hasOwnProperty.call(message, "timeCreated")) writer.uint32(26).string(message.timeCreated);
					return writer;
				};
				/**
				...
				*/
				HistoryMessages.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.HistoryMessages();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
					...
					protobuf.DoubleValue = (function() {
						...
						*/
						DoubleValue.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(9).double(message.value);
							return writer;
						};
						/**
						...
						*/
						DoubleValue.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.DoubleValue();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.FloatValue = (function() {
						...
						*/
						FloatValue.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(13).float(message.value);
							return writer;
						};
						/**
						...
						*/
						FloatValue.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.FloatValue();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.Int64Value = (function() {
						...
						*/
						Int64Value.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(8).int64(message.value);
							return writer;
						};
						/**
						...
						*/
						Int64Value.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.Int64Value();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.UInt64Value = (function() {
						...
						*/
						UInt64Value.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(8).uint64(message.value);
							return writer;
						};
						/**
						...
						*/
						UInt64Value.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.UInt64Value();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.Int32Value = (function() {
						...
						*/
						Int32Value.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(8).int32(message.value);
							return writer;
						};
						/**
						...
						*/
						Int32Value.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.Int32Value();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.UInt32Value = (function() {
						...
						*/
						UInt32Value.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(8).uint32(message.value);
							return writer;
						};
						/**
						...
						*/
						UInt32Value.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.UInt32Value();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.BoolValue = (function() {
						...
						*/
						BoolValue.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(8).bool(message.value);
							return writer;
						};
						/**
						...
						*/
						BoolValue.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.BoolValue();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.StringValue = (function() {
						...
						*/
						StringValue.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(10).string(message.value);
							return writer;
						};
						/**
						...
						*/
						StringValue.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.StringValue();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
					...
					protobuf.BytesValue = (function() {
						...
						*/
						BytesValue.encode = function encode(message, writer) {
							if (!writer) writer = $Writer.create();
							if (message.value != null && Object.hasOwnProperty.call(message, "value")) writer.uint32(10).bytes(message.value);
							return writer;
						};
						/**
						...
						*/
						BytesValue.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.google.protobuf.BytesValue();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
	...
	createClientMethods = function(_a) {
		...
		var send = function(_a) {
			...
			sendMessage(__assign(__assign(__assign({ messageName: "" }, payload), { messageId }), other));
		};
		...
		var getHistoryRequest = function(data, last, messageId) {
			...
			if (historyClient === null || historyClient === void 0 ? void 0 : historyClient.app) historyProto.app = Object.entries(historyClient.app).reduce(function(acc, _a) {
				var _b;
				var key = _a[0], value = _a[1];
				return __assign(__assign({}, acc), (_b = {}, _b[key] = { value }, _b));
			}, {});
			if (historyClient === null || historyClient === void 0 ? void 0 : historyClient.offset) historyProto.offset = Object.entries(historyClient.offset).reduce(function(acc, _a) {
				var _b;
				var key = _a[0], value = _a[1];
		...
		var sendSystemMessage = function(_a, last, messageId, params) {
			...
			send({
				payload: __assign({
					systemMessage: protoExports.SystemMessage.create({ data: JSON.stringify(data) }),
					messageName: mesName,
					last: last ? 1 : -1
				}, params),
				messageId
		...
		var sendVoice = function(data, last, messageId, mesName, params) {
			...
			return send({
				payload: __assign({
					voice: protoExports.Voice.create({ data: new Uint8Array(data) }),
					messageName: mesName,
					last: last ? 1 : -1
				}, params),
				messageId
		...
		var batch = function(cb) {
			...
			var checkLastMessageStatus = function(last) {
				if (lastMessageSent) if (last) throw new Error("Can't send two last items in batch");
				else throw new Error("Can't send messages in batch after last message have been sent");
				else if (last) lastMessageSent = true;
			};
			var upgradedSendText = function() {
				var _a = [];
				for (var _i = 0; _i < arguments.length; _i++) _a[_i] = arguments[_i];
	...
	compileBasePayload = function(_a) {
	...
	appendHeader = function(uint8Array) {
		var arrayBuffer = /* @__PURE__ */ new ArrayBuffer(4);
		new DataView(arrayBuffer, 0).setInt32(0, uint8Array.length, true);
	...
	removeHeader = function(uint8Array) {
		return new Uint8Array(uint8Array).slice(4);
	};
	createProtocol = function(transport, _a) {
		var logger = _a.logger, getToken = _a.getToken, getInitialMeta = _a.getInitialMeta;
		var configuration = __assign(__assign({}, __rest(_a, [
			"logger",
			"getToken",
			"getInitialMeta"
		])), { token: "" });
		...
		var destroyed = false;
		var clearReadyTimer;
		var cancelUpdatingSettingsWhenSocketReady = function() {};
		...
		var send = function(message) {
			...
			var encodedMessageWithHeader = appendHeader(protoExports.Message.encode(createdMessage).finish());
			transport.send(encodedMessageWithHeader);
			emit("outcoming", createdMessage);
		};
		var sendMessage = function(message) {
			if (status === "ready" || message.messageId === initMessageId) try {
				send(message);
				return;
			} catch (_a) {}
			messageQueue.push(message);
			if (status === "closed" && !destroyed) transport.open(url);
		};
		var _c = createClientMethods({
			getMessageId,
			sendMessage
		}), sendDeviceOriginal = _c.sendDevice, sendInitialSettingsOriginal = _c.sendInitialSettings, getHistoryRequestOriginal = _c.getHistoryRequest, sendCancel = _c.sendCancel, sendLegacyDeviceOriginal = _c.sendLegacyDevice, sendMute = _c.sendMute, sendSettingsOriginal = _c.sendSettings, sendText = _c.sendText, sendSystemMessage = _c.sendSystemMessage, sendVoice = _c.sendVoice, batch = _c.batch;
		...
		var sendInitialSettings = (function(data) {
			var args = [];
			for (var _i = 1; _i < arguments.length; _i++) args[_i - 1] = arguments[_i];
			if (data.device && data.settings) currentSettings = __assign(__assign({}, currentSettings), {
				device: data.device,
				settings: data.settings,
				locale: data.locale || void 0
			});
				...
				return __generator(this, function(_h) {
					switch (_h.label) {
						...
						case 4:
							...
							if (!(version < 3)) return [3, 5];
							if (version === 1 && currentSettings.legacyDevice) sendLegacyDevice(currentSettings.legacyDevice, false, initMessageId);
							else if (version === 2 && currentSettings.device) sendDevice(currentSettings.device, false, initMessageId);
							sendSettingsOriginal(currentSettings.settings, true, initMessageId);
							return [3, 9];
						case 5:
							if (!getInitialMeta) return [3, 7];
							return [4, getInitialMeta()];
						case 6:
							_e = _h.sent();
							return [3, 8];
						case 7:
						...
						case 8:
							...
							sendInitialSettings({
								...
								locale: version > 3 ? currentSettings.locale : void 0
		...
		subscriptions.push(transport.on("message", function(message) {
			...
			if (decodedMessage.status) transport.close();
		}));
		return {
			clearQueue: function() {
			...
			reconnect: function() {
				if (status !== "closed") transport.reconnect(url);
				else transport.open(url);
			},
			init: function() {
				if (status === "ready" && window.navigator.onLine) return Promise.resolve();
				return new Promise(function(resolve, reject) {
					var subs = [];
...
//#endregion
//#region node_modules/@salutejs/client/esm/record-0905f37a.js
var createBaseRecorder, CURRENT_VERSION$1, createRecordOfflinePlayer, createOnlineRecordPlayer, lodash_clonedeep, lodash_clonedeepExports, cloneDeep, getDefaultRecord$1, getMid, getRequestKey, normalizeMessage, serializeMessage, createAnswerFromMockByMessageGetter, createMockRecorder, createMockWSCreator;
var init_record_0905f37a = __esmMin((() => {
	init_common_ba25e019();
	init_sdk_7c3cb15e();
	createBaseRecorder = function(isActive, getDefaultRecord) {
	...
	createRecordOfflinePlayer = function(record, _a) {
		var _c = (_a === void 0 ? {} : _a).context, context = _c === void 0 ? window : _c;
		...
		var playMessage = function(message, onPlay) {
			...
			for (var _i = 0, _b = message.items || []; _i < _b.length; _i++) {
				...
				if (item.command) {
					...
					onPlay ? onPlay(command) : !((_a = context.AssistantClient) === null || _a === void 0) && _a.onData && context.AssistantClient.onData(command);
				}
			}
		};
		var playNext = function(onPlay) {
			var _a;
			if (!currentRecord || entryCursor + 1 >= currentRecord.entries.length) return false;
			...
			while ((entry.type !== "incoming" || ((_a = entry.message) === null || _a === void 0 ? void 0 : _a.data) == null || entry.message.name !== MessageNames.ANSWER_TO_USER || !(entry.message.data.items || []).some(function(_a) {
				return _a.command != null;
			})) && entryCursor < currentRecord.entries.length) entry = currentRecord.entries[entryCursor++];
			if (entry.type === "incoming" && entryCursor <= currentRecord.entries.length) entry.message && playMessage(entry.message.data, onPlay);
		...
		var play = function(onPlay) {
			var _a;
			!((_a = context.AssistantClient) === null || _a === void 0) && _a.onStart && context.AssistantClient.onStart();
			if (!currentRecord) return;
			var end = false;
			while (!end) end = !playNext(onPlay);
		};
		var getNextAction = function() {
			var _a, _b, _c, _d, _e, _f, _g;
			if (!currentRecord || entryCursor + 1 >= currentRecord.entries.length) return;
			var cursor = entryCursor;
			var entry = currentRecord.entries[cursor++];
			while (entry.type === "outcoming" && ((_c = (_b = (_a = entry.message) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.systemMessage) === null || _c === void 0 ? void 0 : _c.data) == null && cursor < currentRecord.entries.length) entry = currentRecord.entries[cursor++];
			if (cursor >= currentRecord.entries.length) return;
			return {
		...
		var setRecord = function(rec) {
			if (rec.version !== CURRENT_VERSION$1) throw new Error("Unsupported log version");
			currentRecord = rec;
			entryCursor = 0;
		};
		return {
	...
	createOnlineRecordPlayer = function(record, _a) {
		var _c = (_a === void 0 ? {} : _a).context, context = _c === void 0 ? window : _c;
		var currentRecord = record;
		var entryCursor = 0;
		var playServerAction = function(action, message) {
			var _a;
			(_a = window.AssistantHost) === null || _a === void 0 || _a.sendDataContainer(JSON.stringify({
				action,
		...
		var playNext = function() {
			...
			if (!currentRecord || entryCursor + 1 >= currentRecord.entries.length) return false;
			var entry = currentRecord.entries[entryCursor++];
			while (entry.type !== "outcoming" || (entry.message == null || ((_a = entry.message.data) === null || _a === void 0 ? void 0 : _a.server_action) == null || entry.message.name === "OPEN_ASSISTANT") && entry.text == null && entryCursor < currentRecord.entries.length) entry = currentRecord.entries[entryCursor++];
	...
	*/
	...
	lodash_clonedeepExports = lodash_clonedeep.exports;
	cloneDeep = /* @__PURE__ */ getDefaultExportFromCjs(lodash_clonedeepExports);
	getDefaultRecord$1 = function() {
		return {
			midToRequestKey: {},
			requestKeyToMessages: {}
		};
	};
	getMid = function(message) {
		return String(message.messageId);
	};
	getRequestKey = function(message) {
		var _a, _b;
		if (message.messageName === "OPEN_ASSISTANT") return message.messageName;
		if (message.text && message.text.data) return message.text.data;
		if (message.initialSettings) return "settings.device=" + ((_a = message.initialSettings.device) === null || _a === void 0 ? void 0 : _a.surface);
		var data = (_b = message.systemMessage) === null || _b === void 0 ? void 0 : _b.data;
		if (typeof data === "string") {
			var systemMessageData = JSON.parse(data);
			if (systemMessageData.server_action) return JSON.stringify(systemMessageData.server_action);
		}
		console.error("Не получается определить requestKey для запроса", message);
	};
	normalizeMessage = function(message) {
		if (!message.systemMessage) return message;
		if (typeof message.systemMessage.data === "string") message.systemMessage.data = JSON.parse(message.systemMessage.data);
		return message;
	};
	serializeMessage = function(message) {
		if (message.systemMessage) message.systemMessage = protoExports.SystemMessage.create({ data: JSON.stringify(message.systemMessage.data) });
		return message;
	};
	createAnswerFromMockByMessageGetter = function(record) {
		return function(message) {
			var requestKey = getRequestKey(message);
			if (!requestKey) return;
			var messagesFromMock = record.requestKeyToMessages[requestKey];
			if (!messagesFromMock) return;
			return messagesFromMock.map(function(messageFromMock) {
				return serializeMessage(cloneDeep(messageFromMock));
			});
		};
	};
	createMockRecorder = function(defaultActive) {
		...
		var getRecord = function() {
			...
			var requestKeyToMessages = Object.keys(baseRecord.requestKeyToMessages).sort().reduce(function(result, key) {
				result[key] = baseRecord.requestKeyToMessages[key];
				return result;
			}, {});
			return {
		...
		return {
			...
			stop,
			getRecord,
			handler: prepareHandler(function(entry) {
				switch (entry.type) {
					case "outcoming":
					...
					case "incoming":
						updateRecord(function(record) {
							if (entry.message.messageName === "TTS") return;
							var message = cloneDeep(entry.message);
							...
							var messagesFromMock = record.requestKeyToMessages[requestKey];
							if (messagesFromMock) messagesFromMock.push(normalizedMessage);
							else record.requestKeyToMessages[requestKey] = [normalizedMessage];
	...
	createMockWSCreator = function(recordMock, timeout) {
		...
		return function(url) {
			...
			var send = function(dataToSend) {
				if (!(dataToSend instanceof Uint8Array)) {
					console.error(/* @__PURE__ */ new Error("Неверный тип данных на входе ws.send()"));
					return;
				}
				...
				responseMessageList.forEach(function(responseMessageFromMock) {
					...
					var data = appendHeader(protoExports.Message.encode(responseMessageFromMock).finish());
					setTimeout(function() {
...
//#endregion
//#region node_modules/@salutejs/client/esm/mock.js
init_createAssistant();
init_record_0905f37a();
var createAssistantHostMock = function(_a) {
	...
	var handleAction = function(action, name, requestId) {
		if (!context.AssistantClient || !context.AssistantClient.onRequestState || !context.AssistantClient.onData) throw new Error("Assistant not initialized");
		if (currentResolve) {
		...
		if ("action_id" in action) {
			...
			var handler = handlers.has(actionType) ? handlers.get(actionType) : void 0;
	...
	context.AssistantHost = {
		...
		ready: function() {
			var _a, _b;
			!((_a = window.AssistantClient) === null || _a === void 0) && _a.onStart && ((_b = window.AssistantClient) === null || _b === void 0 || _b.onStart());
			onReady && onReady();
		},
	...
	var addActionHandler = function(actionType, handler) {
		...
		if (handlers.has(type)) throw new Error("Action-handler already exists");
		handlers.set(type, handler);
	};
	/** Удалить обработчик клиентского экшена */
	var removeActionHandler = function(actionType) {
		var type = actionType.toLowerCase();
		if (handlers.has(type)) handlers.delete(type);
	};
	/** Вызвать обработчик команды бека */
	var receiveCommand = function(command) {
		if (!context.AssistantClient || !context.AssistantClient.onData) throw new Error("Assistant not initialized");
...
var createAssistantHostMockWithRecord = function(_a) {
	...
	var mock = createAssistantHostMock({ context });
	var player = createRecordOfflinePlayer(record, { context });
	var hasNext = true;
...
//#endregion
//#region node_modules/@salutejs/client/esm/client-ed11c12c.js
function convertFieldValuesToString(object) {
	return Object.keys(object).reduce(function(acc, key) {
		if (object[key]) acc[key] = typeof object[key] === "string" && object[key].startsWith("{") ? object[key] : JSON.stringify(object[key]);
		return acc;
	}, {});
}
var asr, asrExports, mtt, mttExports, createClient$1, RETRY_INTERVAL, defaultWSCreator, createTransport, convertToMetaPermissions, getLocation, getTime, getAnswerForRequestPermissions, DEFAULT_PROJECT_ID, DEFAULT_APPLICATION_ID, DEFAULT_APP_VERSION_ID, DEFAULT_APP, BASIC_SMART_APP_COMMANDS_TYPES, isDefaultApp, promiseTimeout, createClient;
var init_client_ed11c12c = __esmMin((() => {
	init_common_ba25e019();
	init_sdk_7c3cb15e();
	asr = { exports: {} };
	(function(module) {
			...
			$root.Variables = (function() {
				...
				*/
				Variables.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Variables(), key, value;
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.UndecodedSeconds = (function() {
				...
				*/
				UndecodedSeconds.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.UndecodedSeconds();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.FullyFinalized = (function() {
				...
				*/
				FullyFinalized.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.FullyFinalized();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							default:
			...
			$root.EmotionResult = (function() {
				...
				*/
				EmotionResult.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.EmotionResult();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.Hypothesis = (function() {
				...
				*/
				Hypothesis.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Hypothesis();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.DecoderResult = (function() {
				...
				*/
				DecoderResult.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.DecoderResult();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
				...
				DecoderResult.ContextAnswer = (function() {
					...
					*/
					ContextAnswer.decode = function decode(reader, length) {
						if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
						var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.DecoderResult.ContextAnswer();
						while (reader.pos < end) {
							var tag = reader.uint32();
							switch (tag >>> 3) {
								case 1:
					...
					ContextAnswer.ContextRef = (function() {
						...
						*/
						ContextRef.decode = function decode(reader, length) {
							if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
							var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.DecoderResult.ContextAnswer.ContextRef();
							while (reader.pos < end) {
								var tag = reader.uint32();
								switch (tag >>> 3) {
									case 1:
			...
			$root.ErrorResponse = (function() {
				...
				*/
				ErrorResponse.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.ErrorResponse();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
			...
			$root.PacketWrapperFromServer = (function() {
				...
				*/
				PacketWrapperFromServer.decode = function decode(reader, length) {
					if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
					var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.PacketWrapperFromServer();
					while (reader.pos < end) {
						var tag = reader.uint32();
						switch (tag >>> 3) {
							case 1:
				...
				*/
				...
				Music2TrackProtocol.DecoderResult = (function() {
					...
					*/
					DecoderResult.decode = function decode(reader, length) {
						if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
						var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Music2TrackProtocol.DecoderResult();
						while (reader.pos < end) {
							var tag = reader.uint32();
							switch (tag >>> 3) {
								case 1:
				...
				Music2TrackProtocol.ErrorResponse = (function() {
					...
					*/
					ErrorResponse.decode = function decode(reader, length) {
						if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
						var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Music2TrackProtocol.ErrorResponse();
						while (reader.pos < end) {
							var tag = reader.uint32();
							switch (tag >>> 3) {
								case 1:
				...
				Music2TrackProtocol.MttResponse = (function() {
					...
					*/
					MttResponse.decode = function decode(reader, length) {
						if (!(reader instanceof $Reader)) reader = $Reader.create(reader);
						var end = length === void 0 ? reader.len : reader.pos + length, message = new $root.Music2TrackProtocol.MttResponse();
						while (reader.pos < end) {
							var tag = reader.uint32();
							switch (tag >>> 3) {
								case 1:
	...
	mttExports = mtt.exports;
	createClient$1 = function(protocol, provideMeta, _a) {
		if (provideMeta === void 0) provideMeta = void 0;
				...
				return __generator(this, function(_d) {
					switch (_d.label) {
						case 0:
							...
							if (!provideMeta) return [3, 2];
							return [4, provideMeta()];
						case 1:
							_c = _d.sent();
							return [3, 3];
						case 2:
						...
						case 3:
							meta = _c;
							return [2, waitForAnswer(sendData(data, "OPEN_ASSISTANT", meta))];
					}
				});
			});
		};
			...
			return __awaiter(void 0, void 0, void 0, function() {
				...
				return __generator(this, function(_b) {
					switch (_b.label) {
						case 0:
							if (!provideMeta) return [3, 2];
							return [4, provideMeta(additionalMeta)];
						case 1:
							_a = _b.sent();
							return [3, 3];
						case 2:
						...
						case 3:
							meta = _a;
							if (typeof meta !== "undefined") sendSystemMessage({
								data: {},
								messageName: ""
							}, false, { meta });
							return [2];
				...
				return __generator(this, function(_a) {
					switch (_a.label) {
						case 0:
							...
							return [4, sendMeta(function(data, _, _a) {
		...
		var sendText = function(text, isSsml, shouldSendDisableDubbing, additionalMeta) {
			...
			return __awaiter(void 0, void 0, void 0, function() {
				return __generator(this, function(_a) {
					if (text.trim() === "") return [2, void 0];
					return [2, protocol.batch(function(_a) {
						...
						return __awaiter(void 0, void 0, void 0, function() {
							...
							return __generator(this, function(_b) {
								switch (_b.label) {
									case 0: return [4, sendMeta(sendSystemMessage, additionalMeta)];
									case 1:
										_b.sent();
										prevDubbing = protocol.configuration.settings.dubbing;
										sendDisableDubbing = prevDubbing !== -1 && shouldSendDisableDubbing;
										if (!sendDisableDubbing) return [3, 3];
										return [4, sendSettings({ dubbing: -1 }, false)];
									case 2:
										_b.sent();
										_b.label = 3;
									case 3:
										...
										if (sendDisableDubbing && isStillNeedReturnDubbing) sendSettings({ dubbing: prevDubbing });
										return [2, messageId];
								}
							});
						});
		...
		* отправляет мету */
				...
				return __awaiter(void 0, void 0, void 0, function() {
					...
					return __generator(this, function(_b) {
						switch (_b.label) {
							case 0:
								first = true;
								return [4, sendMeta(sendSystemMessage, additionalMeta)];
							case 1:
								_b.sent();
								return [4, callback({
									sendVoice: function() {
		...
		var off = [protocol.on("incoming", function(message) {
			...
			if ((_a = message.systemMessage) === null || _a === void 0 ? void 0 : _a.data) emit("systemMessage", JSON.parse(message.systemMessage.data), message);
			if (message.voice) emit("voice", message.voice.data || new Uint8Array(), message);
			if (message.status) emit("status", message.status, message);
			if (message.messageName === "TAKE_HISTORY" && ((_b = message.bytes) === null || _b === void 0 ? void 0 : _b.data)) emit("history", protoExports.GetHistoryResponse.decode((_c = message.bytes) === null || _c === void 0 ? void 0 : _c.data).historyMessages.map(function(historyMessage) {
				return __assign(__assign({}, historyMessage), { content: JSON.parse(historyMessage.content || "") });
			}), message);
			if (message.messageName === MessageNames.STT && (message.text || ((_e = (_d = message.bytes) === null || _d === void 0 ? void 0 : _d.data) === null || _e === void 0 ? void 0 : _e.length))) {
				var response = ((_g = (_f = message.bytes) === null || _f === void 0 ? void 0 : _f.data) === null || _g === void 0 ? void 0 : _g.length) ? asrExports.PacketWrapperFromServer.decode(message.bytes.data) : void 0;
				emit("stt", {
					text: message.text,
					response
				}, message);
			}
			if (message.messageName === MessageNames.MTT && ((_j = (_h = message.bytes) === null || _h === void 0 ? void 0 : _h.data) === null || _j === void 0 ? void 0 : _j.length)) emit("musicRecognition", mttExports.Music2TrackProtocol.MttResponse.decode(message.bytes.data), message);
		}), protocol.on("ready", function() {
			emit("ready");
		})];
		return {
			destroy: function() {
	...
	RETRY_INTERVAL = 300;
	defaultWSCreator = function(url) {
		return new WebSocket(url);
	};
	createTransport = function(_a) {
		...
		var close = function() {
			...
			status = "closing";
			webSocket === null || webSocket === void 0 || webSocket.close();
		};
				...
				return __generator(this, function(_a) {
					switch (_a.label) {
						case 0:
							status = "connecting";
							emit("connecting");
							if (!(!hasCert && window.navigator.onLine)) return [3, 2];
							return [4, checkCert(checkCertUrl)];
						case 1:
							okay = _a.sent();
							if (!okay) {
								status = "closed";
								emit("close");
								emit("error", /* @__PURE__ */ new Error("Cert authority invalid"));
								return [2];
							}
							hasCert = true;
							_a.label = 2;
						case 2:
							...
							webSocket.addEventListener("error", function(e) {
								if (status !== "connecting") throw e;
								if (!webSocket || webSocket.readyState === 3 && !stopped) {
									window.clearTimeout(retryTimeoutId);
									if (retries < 2) retryTimeoutId = window.setTimeout(function() {
										open(url);
										retries++;
	...
	convertToMetaPermissions = function(permission) {
	...
	getTime = function() {
		return {
			...
			timezone_offset_sec: -(/* @__PURE__ */ new Date()).getTimezoneOffset() * 60,
			timestamp: Date.now()
		};
	};
	getAnswerForRequestPermissions = function(requestMessageId, appInfo, items) {
		return __awaiter(void 0, void 0, void 0, function() {
			...
			return __generator(this, function(_a) {
				...
				response = {
					...
					server_action: {
						...
						command_response: { request_permissions: { permissions: [] } }
					}
				};
				return [2, Promise.all(items.map(function(permission) {
					return __awaiter(void 0, void 0, void 0, function() {
						var _a, location_1;
						var _c, _d, _e;
						return __generator(this, function(_f) {
							switch (_f.label) {
								...
								case 1:
									...
									return [4, getLocation()];
								case 2:
									location_1 = _f.sent();
									permissions.geo = "granted";
									response.meta.location = location_1;
									(_c = response.server_action.command_response.request_permissions) === null || _c === void 0 || _c.permissions.push({
										type: "geo",
										status: "granted"
									});
									return [3, 4];
								case 3:
									...
									(_d = response.server_action.command_response.request_permissions) === null || _d === void 0 || _d.permissions.push({
										type: "geo",
								...
								case 4: return [3, 6];
								case 5:
									(_e = response.server_action.command_response.request_permissions) === null || _e === void 0 || _e.permissions.push({
										type: permission,
										status: "denied_permanently"
									});
									return [3, 6];
	...
	DEFAULT_PROJECT_ID = "d929986a-611a-2ba0-6174-1928c99600a5";
	DEFAULT_APPLICATION_ID = "7c4e23bf-cd93-b57e-874b-d9fc1b35f93d";
	...
	DEFAULT_APP = {
		...
		frontendStateId: [
			...
			DEFAULT_APP_VERSION_ID
		].join("_"),
		frontendType: "DIALOG",
		systemName: "assistant",
		frontendEndpoint: "None"
	};
	BASIC_SMART_APP_COMMANDS_TYPES = [
		"smart_app_data",
		"smart_app_error",
		"start_smart_search",
		"navigation"
	];
	isDefaultApp = function(appInfo) {
		return appInfo.frontendStateId === DEFAULT_APP.frontendStateId;
	};
	promiseTimeout = function(promise, timeout) {
		var timeoutId;
		return Promise.race([promise.then(function(v) {
			...
			timeoutId = window.setTimeout(function() {
				reject(/* @__PURE__ */ new Error("Timed out in " + timeout + " ms."));
			}, timeout);
		})]);
	};
	createClient = function(configuration) {
		var metaProvider = function() {
			return __awaiter(void 0, void 0, void 0, function() {
				return __generator(this, function(_a) {
					return [2, Promise.resolve(convertFieldValuesToString({
						theme: "dark",
						time: getTime(),
						current_app: { app_info: DEFAULT_APP }
					}))];
				});
			});
		};
		return createClient$1(createProtocol(createTransport({}), __assign(__assign({}, configuration), { getInitialMeta: function() {
			return convertFieldValuesToString({ default_character: "sber" });
		} })), metaProvider, { getVoiceMeta: function() {
			return {};
		} });
	};
}));
//#endregion
//#region node_modules/@salutejs/client/esm/voicePlayer-705b49ef.js
function createSilentAudioFile(sampleRate) {
	var arrayBuffer = /* @__PURE__ */ new ArrayBuffer(10);
	var dataView = new DataView(arrayBuffer);
	dataView.setUint32(0, sampleRate, true);
	dataView.setUint32(4, sampleRate, true);
	dataView.setUint16(8, 1, true);
	return "data:audio/wav;base64,UklGRisAAABXQVZFZm10IBAAAAABAAEA" + window.btoa(String.fromCharCode.apply(null, new Uint8Array(arrayBuffer))).slice(0, 13) + "AgAZGF0YQcAAACAgICAgICAAAA=";
}
var IS_APPLE_MOBILE, createIosSilentModePatch, iosSilentModePatch, createTrackCollection, createChunkQueue, parseWavHeader, HZ_BYTES_COUNT, from16BitToFloat32, createTrackStream, createVoicePlayer;
var init_voicePlayer_705b49ef = __esmMin((() => {
	init_common_ba25e019();
	IS_APPLE_MOBILE = typeof window !== "undefined" ? (/iPad|iPhone|iPod/.test(navigator.platform) || navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1) && !window.MSStream : false;
	createIosSilentModePatch = function() {
		var audio = null;
		...
		*/
		var initialize = function() {
			...
			audio.preload = "auto";
			audio.loop = true;
			audio.src = createSilentAudioFile(16e3);
			audio.load();
		};
		...
		return {
			...
			turnOff,
			turnOn: function() {
				audio === null || audio === void 0 || audio.play().catch(function(e) {
	...
	iosSilentModePatch = createIosSilentModePatch();
	createTrackCollection = function() {
		var trackIds;
		var trackMap;
		var clear = function() {
			trackIds = new Array();
		...
		var push = function(id, track) {
			if (trackMap.has(id)) throw new Error("Track already exists");
		...
		var getById = function(id) {
			...
			if (track === void 0) throw new Error("Unknown track id");
			return track;
		};
		var getByIndex = function(index) {
			if (index < 0 || index >= trackIds.length) throw new Error("Index out of bounds");
			var track = trackMap.get(trackIds[index]);
			if (track == null) throw new Error("Something wrong...");
			return track;
		};
	...
	HZ_BYTES_COUNT = 2;
	from16BitToFloat32 = function(incomingData) {
		var l = incomingData.length;
		var outputData = new Float32Array(l);
		for (var i = 0; i < l; i += 1) {
			var sample = incomingData[i] / 32768;
	...
	createTrackStream = function(ctx, _a) {
		var _b = _a === void 0 ? {} : _a, _c = _b.sampleRate, sampleRate = _c === void 0 ? 24e3 : _c, _d = _b.numberOfChannels, numberOfChannels = _d === void 0 ? 1 : _d, _e = _b.delay, delay = _e === void 0 ? 0 : _e, onPlay = _b.onPlay, onEnd = _b.onEnd, onStop = _b.onStop, trackStatus = _b.trackStatus;
		...
		var createChunk = function(chunk) {
			...
			for (var i = 0; i < numberOfChannels; i++) {
				...
				var index = 0;
				for (var j = i; j < chunk.length; j += numberOfChannels) channelChunk[index++] = chunk[j];
		...
		var getChunkFromBuffer = function() {
			...
			if (bytesArraysSizes.prepend != null) bufferUi8[0] = bytesArraysSizes.prepend;
		...
		var write = function(data) {
			...
			if (firstChunk) {
				...
				if (wavHeader) {
					...
					if (wavHeader.sampleRate !== sampleRate) console.warn("WAV файл содержит sampleRate " + wavHeader.sampleRate + ", но ожидался " + sampleRate);
					if (wavHeader.channels !== numberOfChannels) console.warn("WAV файл содержит " + wavHeader.channels + " каналов, но ожидался " + numberOfChannels);
				} else slicePoint = 0;
				firstChunk = false;
			}
			if (slicePoint >= data.length) return;
	...
	createVoicePlayer = function(actx, _a) {
		var _b = _a === void 0 ? {} : _a, _c = _b.startVoiceDelay, startVoiceDelay = _c === void 0 ? .2 : _c, sampleRate = _b.sampleRate, numberOfChannels = _b.numberOfChannels;
		...
		var play = function() {
			...
			if (!iosSilentModePatch.isActive) iosSilentModePatch.turnOn();
		...
		var append = function(data, trackId, last) {
			...
			var current = tracks.has(trackId) ? tracks.get(trackId) : void 0;
			if (current == null) {
			...
			if (current.status !== "end" && data.length) current.write(data);
			if (last) current.setLoaded();
			play();
		};
		var stop = function() {
...
//#endregion
//#region node_modules/@salutejs/client/esm/assistant-c0a66ca1.js
/**
...
function createAudioContext(options) {
	if (window.AudioContext) return new AudioContext(options);
	if (window.webkitAudioContext) return new window.webkitAudioContext();
	throw new Error("Audio not supported");
}
var isAudioSupported, _a, on, emit, audioContext, resolveAudioContext, downsampleBuffer, TARGET_SAMPLE_RATE, IS_FIREFOX, IS_SAFARI, context, processor, analyser, destination, createAudioRecorder, createNavigatorAudioProvider, createVoiceListener, filterEmptyChunks, createVoice, createMutexedObject, createMutexSwitcher, createAssistant$1;
var init_assistant_c0a66ca1 = __esmMin((() => {
	init_common_ba25e019();
	init_client_ed11c12c();
	init_sdk_7c3cb15e();
	init_voicePlayer_705b49ef();
	isAudioSupported = typeof window !== "undefined" && (window.AudioContext || window.webkitAudioContext);
	_a = createNanoEvents(), on = _a.on, emit = _a.emit;
	resolveAudioContext = function(onReady) {
		if (!audioContext) {
			...
			if (!audioContext.ready) {
				var handleClick_1 = function() {
					...
					if (audioContext.context.state === "suspended") audioContext.context.resume();
				};
				document.addEventListener("click", handleClick_1);
			}
		}
		if (audioContext.ready) onReady && onReady(audioContext.context, function() {
			return iosSilentModePatch.destroy();
		});
		else var unsubscribe_1 = on("ready", function() {
	...
	downsampleBuffer = function(buffer, inSampleRate, outSampleRate) {
		if (outSampleRate > inSampleRate) throw new Error("downsampling rate show be smaller than original sample rate");
		...
		while (offsetResult < result.length) {
			...
			result[offsetResult] = Math.min(1, accum / count) * 32767;
			offsetResult++;
	...
	analyser = null;
	createAudioRecorder = function(stream, cb, targetSampleRate, useAnalyser) {
		return new Promise(function(resolve) {
			var state = "inactive";
			var input;
			var stop = function() {
			...
			var start = function() {
				if (state !== "inactive") throw new Error("Can't start not inactive recorder");
				state = "recording";
				if (!context) context = createAudioContext({ sampleRate: IS_FIREFOX ? void 0 : targetSampleRate });
				input = context.createMediaStreamSource(stream);
				if (!processor) processor = context.createScriptProcessor(2048, 1, 1);
				if (!analyser && useAnalyser) {
					analyser = context.createAnalyser();
					analyser.fftSize = 1024;
				}
				var listener = function(e) {
					var data = downsampleBuffer(e.inputBuffer.getChannelData(0), context.sampleRate, targetSampleRate);
					...
					if (!IS_SAFARI || new Uint8Array(data).some(function(voiceData) {
						...
						if (analyser) {
							...
							analyser === null || analyser === void 0 || analyser.getByteFrequencyData(analyserArray);
						}
						cb(data, analyserArray, last);
						resolve(stop);
					}
					if (last) processor.removeEventListener("audioprocess", listener);
				};
				processor.addEventListener("audioprocess", listener);
				input.connect(processor);
				if (analyser) input.connect(analyser);
				if (!destination) destination = context.createMediaStreamDestination();
				processor.connect(destination);
			};
			start();
		});
	};
	createNavigatorAudioProvider = function(cb, useAnalyser, options) {
		...
		return navigator.mediaDevices.getUserMedia({ audio: {
			...
			if (window.location.protocol === "http:") throw new Error("Audio is supported only on a secure connection");
			throw err;
		});
	};
	createVoiceListener = function(createAudioProvider) {
		...
		var stop = function() {
			...
			status = "stopped";
			stopRecord === null || stopRecord === void 0 || stopRecord();
			emit("status", "stopped");
		};
		var listen = function(handleVoice) {
			...
			return createAudioProvider(function(data, analyser, last) {
				...
				if (capturedToken.current === true || status === "stopped") stopRecord();
				else {
					status = "listen";
					emit("status", "listen");
				}
	...
	filterEmptyChunks = function(chunksOriginal) {
	...
	createVoice = function(client, settings, emit, onReady) {
		...
		var stopVoice = function(sendCancel) {
			...
			streaming = null;
			if (sendCancel && currentVoiceMessageId) client.sendCancel(currentVoiceMessageId);
		...
		var stop = function() {
			stopVoice();
			voicePlayer === null || voicePlayer === void 0 || voicePlayer.stop();
		};
		var recognize = function(_a) {
			...
			return __awaiter(void 0, void 0, void 0, function() {
				...
				return __generator(this, function(_d) {
					switch (_d.label) {
						case 0:
							if (stopVoice()) return [2];
							if (isPlaying) {
								voicePlayer === null || voicePlayer === void 0 || voicePlayer.stop();
								return [2];
							}
							if (settings.current.disableListening) return [2];
							if (!(listener.status === "stopped" && !isRecognizeInitializing)) return [3, 2];
							isRecognizeInitializing = true;
							unsubscribe_1 = listener.on("status", function() {
								isRecognizeInitializing = false;
								unsubscribe_1();
							});
							return [4, client.init().catch(function(error) {
								isRecognizeInitializing = false;
								throw error;
							})];
						case 1:
							_d.sent();
							return [2, client.createVoiceStream(function(_a) {
								var sendVoice = _a.sendVoice, messageId = _a.messageId;
								begin === null || begin === void 0 || begin.forEach(function(chunk) {
								...
								return listener.listen(function(chunk, analyser, last) {
									if (analyser) emit({ voiceAnalyser: { data: analyser } });
									sendVoice(chunk, last, messageName);
								});
							}, { source: { sourceType: isAutoListening ? "autoListening" : "lavashar" } })];
						case 2: return [2];
					}
				});
			});
				...
				return __generator(this, function(_a) {
					switch (_a.label) {
						case 0:
							...
							if (streaming === null || streaming === void 0 ? void 0 : streaming(chunks, last)) return [2];
							stopVoice();
							if (isPlaying) voicePlayer === null || voicePlayer === void 0 || voicePlayer.stop();
							if (!(!isRecognizeInitializing && chunks.length)) return [3, 2];
							isRecognizeInitializing = true;
								...
								return __awaiter(void 0, void 0, void 0, function() {
									...
									return __generator(this, function(_b) {
										...
										streaming = function(ch, l) {
											...
											if (l) streaming = null;
											else cancelTimeoutId = setTimeout(function() {
												if (streaming) stopVoice();
											}, 3e3);
											return true;
										};
		...
		*/
		...
		if (isAudioSupported) resolveAudioContext(function(context, destroy) {
			...
			subscriptions.push(voicePlayer.on("end", function(mesId) {
				...
				if (mesId === autolistenMessageId) listen().catch(function(error) {
					console.error(error);
				});
				var idx = 0;
				do
		...
		subscriptions.push(client.on("voice", function(data, message) {
			if (settings.current.disableDubbing) return;
			voicePlayer === null || voicePlayer === void 0 || voicePlayer.append(data, message.messageId.toString(), message.last === 1);
		}), listener.on("status", function(status) {
			emit({ listener: { status } });
			if (status === "listen") {
				voicePlayer === null || voicePlayer === void 0 || voicePlayer.setActive(false);
				emit({ emotion: "listen" });
			} else if (status === "stopped") {
				voicePlayer === null || voicePlayer === void 0 || voicePlayer.setActive(!settings.current.disableDubbing);
				emit({
					asr: { text: "" },
					emotion: "idle"
				});
			...
			if (autoListening) if (settings.current.disableDubbing === false) autolistenMessageId = messageId;
			else listen({}, autoListening).catch(function(error) {
				console.error(error);
			});
		}), client.on("status", function(_a) {
			if (_a.code < 0) stopVoice(false);
			...
			if (text) {
				...
				if (last || listening) emit({ asr: {
					mid: originalMessage.messageId,
					text: text.data || "",
					last
				} });
				if (last) stopVoice(false);
			}
			if (response) {
				...
				if ((last || listening) && ((_b = decoderResultField === null || decoderResultField === void 0 ? void 0 : decoderResultField.hypothesis) === null || _b === void 0 ? void 0 : _b.length)) emit({ asr: {
					mid: originalMessage.messageId,
					text: decoderResultField.hypothesis[0].normalizedText || "",
					last
				} });
				if (last || errorResponse) stopVoice(false);
			}
			...
			if (((_a = response.decoderResultField) === null || _a === void 0 ? void 0 : _a.isFinal) || response.errorResponse) stopVoice(false);
		}), settings.on("change-request", function(nextSettings) {
			var disableDubbing = nextSettings.disableDubbing;
			nextSettings.disableListening && stopVoice();
			settings.current.disableDubbing !== disableDubbing && (voicePlayer === null || voicePlayer === void 0 || voicePlayer.setActive(!disableDubbing));
		}));
		return {
			destroy: function() {
				stopVoice();
				...
				destroyAudioContext === null || destroyAudioContext === void 0 || destroyAudioContext();
			},
			listen,
			shazam,
			sendVoice,
	...
	createMutexedObject = function(initialObject) {
		var _a = createNanoEvents(), on = _a.on, emit = _a.emit;
		...
		var tryApply = function() {
			if (mode === "released") {
				...
				if (Object.keys(nextObject).some(function(name) {
		...
		Object.keys(initialObject).forEach(function(prop) {
			Object.defineProperty(current, prop, { get: function() {
				return object[prop];
			} });
		});
		return {
	...
	createMutexSwitcher = function(_a, initialDeps) {
		var lock = _a.lock, release = _a.release;
		var deps = __assign({}, initialDeps);
		return { change: function(nextDeps) {
	...
	createAssistant$1 = function(_a) {
		var _b;
		var getMeta = _a.getMeta, getInitialMeta = _a.getInitialMeta, getVoiceMeta = _a.getVoiceMeta, checkCertUrl = _a.checkCertUrl, configuration = __rest(_a, [
			"getMeta",
			"getInitialMeta",
			"getVoiceMeta",
			"checkCertUrl"
		...
		var metaProvider = function(additionalMeta) {
			return __awaiter(void 0, void 0, void 0, function() {
				...
				return __generator(this, function(_b) {
					switch (_b.label) {
						case 0:
							if (!(app !== null && app.info.frontendType === "WEB_APP" && app.getState)) return [3, 2];
							return [4, promiseTimeout(app.getState(), 200).catch(function() {
									...
									return __generator(this, function(_a) {
										switch (_a.label) {
											case 0:
												...
												backgroundAppsMeta = [];
												return [4, Promise.all(backgroundAppsIds.map(function(applicationId) {
													return __awaiter(void 0, void 0, void 0, function() {
														var _a, getState;
														return __generator(this, function(_b) {
															...
															return [2, promiseTimeout(getState(), 200).then(function(state) {
																return state;
															}, function() {
																return {};
															})];
											...
											case 1:
												_a.sent();
												return [2, backgroundAppsMeta];
										}
									});
								});
							};
							return [4, getBackgroundAppsMeta()];
						case 4:
							background_apps = _b.sent();
							return [2, convertFieldValuesToString(__assign(__assign(__assign(__assign({}, sdkMeta), {
								time: getTime(),
								current_app,
								background_apps
		...
		var protocol = createProtocol(transport, __assign(__assign({}, configuration), {
			getInitialMeta: typeof getInitialMeta !== "undefined" ? function() {
				...
				return convertFieldValuesToString({ default_character: defaultCharacter });
			},
			settings: __assign(__assign({}, configuration.settings), { dubbing: -1 })
		}));
		var client = createClient$1(protocol, metaProvider, { getVoiceMeta: function() {
		...
		var voice = createVoice(client, settings, function(event) {
			...
			if (typeof event.listener !== "undefined") settingsSwitcher.change({ isListenerStopped: event.listener.status === "stopped" });
			emit("assistant", event);
		}, function() {
			voiceReady = true;
			if (!settings.current.disableDubbing) protocol.changeSettings({ dubbing: 1 });
		});
		/** завершает текущий апп */
		var closeApp = function(closing) {
			if (closing === void 0) closing = app.info;
			if (closing.applicationId === app.info.applicationId) app = { info: DEFAULT_APP };
			if (!isDefaultApp(closing)) emit("app", {
				type: "close",
				app: closing
			});
		};
		...
		var sendServerAction = function(serverAction, messageName, requestId, actionApp, mode) {
			if (messageName === void 0) messageName = "SERVER_ACTION";
			if (requestId === void 0) requestId = void 0;
			...
			client.sendServerAction(serverAction, actionApp, messageName, mode).then(function(messageId) {
				if (requestId && messageId) requestIdMap[messageId.toString()] = requestId;
			});
		};
		...
		var sendMetaForPermissionRequest = function(requestMessageId, appInfo, items) {
			return __awaiter(void 0, void 0, void 0, function() {
				...
				return __generator(this, function(_b) {
					switch (_b.label) {
						case 0: return [4, getAnswerForRequestPermissions(requestMessageId, appInfo, items)];
						case 1:
							_a = _b.sent(), props = __rest(_a.meta, []), data = __rest(_a, ["meta"]);
							return [4, metaProvider()];
						case 2:
							meta = _b.sent();
		...
		subscriptions.push(protocol.on("ready", function() {
			return emit("vps", { type: "ready" });
		}));
		subscriptions.push(settings.on("changed", function(nextSettings, prevSettings) {
			if (nextSettings.disableDubbing !== prevSettings.disableDubbing) voiceReady && protocol.changeSettings({ dubbing: nextSettings.disableDubbing ? -1 : 1 });
		}));
		...
		subscriptions.push(protocol.on("outcoming", function(message) {
			if (message.text || message.voice) lastMid = message.messageId;
			emit("vps", {
				type: "outcoming",
				message
			});
		...
		subscriptions.push(client.on("systemMessage", function(systemMessage, originalMessage) {
			if (originalMessage.messageName === "ANSWER_TO_USER") {
				...
				if (!isDialog && isAppChanged && (isChatApp || activate_app_info !== false)) emit("app", {
					type: "run",
					app: mesAppInfo
				});
				if (isDialog && isAppChanged && app.info.applicationId !== DEFAULT_APP.applicationId) emit("app", {
					type: "run",
					app: DEFAULT_APP
				});
				if (answerId >= 2 && lastMid > originalMessage.messageId) client.sendCancel(originalMessage.messageId);
				if (lastMid < originalMessage.messageId && items.findIndex(function(_a) {
					var bubble = _a.bubble, card = _a.card;
					return bubble || card;
				}) >= 0) lastMid = originalMessage.messageId;
				if (items.length) {
					var _loop_1 = function(i) {
						...
						if (typeof command !== "undefined") {
							...
							if (command.type === "action") emit("actionCommand", {
								type: "command",
								command,
								appInfo: mesAppInfo
							});
							if (mesAppInfo && BASIC_SMART_APP_COMMANDS_TYPES.includes(command.type)) emit("app", {
								type: "command",
							...
							if (command.type === "close_app" && !isDialog) closeApp(mesAppInfo);
						}
					};
		...
		on("app", function(event) {
			...
			if (event.type === "command") {
				...
				if (Array.isArray(backgroundAppOnCommand)) backgroundAppOnCommand.forEach(function(onCommand) {
					var _a;
		...
		var start = function(_a) {
			var _b = _a === void 0 ? {} : _a, _c = _b.disableGreetings, disableGreetings = _c === void 0 ? false : _c, _d = _b.initPhrase, initPhrase = _d === void 0 ? void 0 : _d, _e = _b.isFirstSession, isFirstSession = _e === void 0 ? false : _e;
			return __awaiter(void 0, void 0, void 0, function() {
				return __generator(this, function(_f) {
					switch (_f.label) {
						case 0:
							if (!(!disableGreetings && isDefaultApp(app.info))) return [3, 2];
							return [4, client.sendOpenAssistant({ isFirstSession })];
						case 1:
							_f.sent();
							_f.label = 2;
						case 2:
							if (initPhrase) return [2, client.sendText(initPhrase).then(function(messageId) {
								return messageId ? client.waitForAnswer(messageId) : void 0;
							})];
							return [2, void 0];
					}
				});
		...
		return {
			...
			stop: function() {
				voice.stop();
				if (lastMid !== 0) client.sendCancel(lastMid);
				setTimeout(function() {
					protocol.clearQueue();
					transport.close();
				});
			...
			addBackgroundApp: function(_a) {
				...
				var onCommand = function(subscriber) {
					var _a;
					(_a = backgroundApps[appInfo.applicationId]) === null || _a === void 0 || _a.commandsSubscribers.push(subscriber);
					return { clearSubscribers: function() {
						if (backgroundApps[appInfo.applicationId]) backgroundApps[appInfo.applicationId].commandsSubscribers = [];
					} };
				};
...
//#endregion
//#region node_modules/@salutejs/client/esm/createAssistantDevOrigin.js
var createAssistantDevOrigin_exports = /* @__PURE__ */ __exportAll({
	createAssistantDev: () => createAssistantDev$1,
	createSmartappDebugger: () => createSmartappDebugger$1
});
var createComponent, FONTS_CDN, fontFace, fontFamily500, fontFamily400, BubbleStyles, BubbleMDStyles, BubbleSMStyles, SphereButtonStyles, SphereButtonMDStyles, SphereButtonSMStyles, CarouselTouchStyles, KeyboardTouchStyles, VoiceTouchStyles, SuggestsStyles, SuggestLGStyles, SuggestMDStyles, SuggestSMStyles, SuggestOutlinedStyles, SuggestFilledStyles, TextInputStyles, TextInputPureStyles, TextInputFilledStyles, NativePanelStyles, NativePanelPaddingsSMStyles, NativePanelPaddingsMDStyles, NativePanelPaddingsLGStyles, NativePanelInputOffsetMDStyles, NativePanelInputOffsetLGStyles, NativePanelTouchStyles, NativePanelDesktopNotscreenshotModeStyles, NativePanelDesktopscreenshotModeStyles, NativePanelDesktopBubblePositionLG, NativePanelDesktopBubblePositionMD, NativePanelDesktopStyles, styles$1, saluteIcon, musicIcon, newsIcon, createCarouselTouchIcon, template$1, cl, render$1, nativePanel, root, renderNativePanel, template, defaultState, render, recordPanel, renderAssistantRecordPanel, createConsoleLogger, CURRENT_VERSION, getDefaultRecord, createLogCallbackRecorder, createRecordDownloader, prefix, createRecoveryStateRepository, SDK_VERSION, APP_VERSION, HOST_APP_ID, FEATURES, legacyDevice, initializeNativeSDKEmulator, stand, channelForSurface, createAssistantDev$1, parseJwt, createSmartappDebugger$1;
var init_createAssistantDevOrigin = __esmMin((() => {
	init_common_ba25e019();
	init_assistant_c0a66ca1();
	init_record_0905f37a();
	createComponent = function(_a) {
		...
		var uneffects = function() {
			Object.values(effects.offs).forEach(function(off) {
				return off();
			});
			effects.deps = {};
			effects.offs = {};
		...
		var unmount = function() {
			uneffects();
			root === null || root === void 0 || root.remove();
			lastProps = null;
			state = {};
			renderMode = "release";
		...
		var update = function(props) {
			...
			if (root && refs) {
				...
				var effect = function(cb, deps) {
					currentDepsId_1 += 1;
					if (!effects.deps[currentDepsId_1]) effects.deps[currentDepsId_1] = {
						arr: [],
						calls: 0
					};
					var _a = effects.deps[currentDepsId_1], prevDeps = _a.arr, calls = _a.calls;
					if (deps.length === 0 && calls === 0 || calls === 0 || prevDeps.some(function(prev, index) {
						return prev !== deps[index];
					})) {
						effects.deps[currentDepsId_1].arr = deps;
						effects.deps[currentDepsId_1].calls += 1;
						if (typeof effects.offs[currentDepsId_1] === "function") effects.offs[currentDepsId_1]();
						var off = cb();
						if (typeof off === "function") effects.offs[currentDepsId_1] = off;
					}
				};
				tryRender(render({
					refs,
	...
	FONTS_CDN = "https://cdn-app.sberdevices.ru/shared-static/0.0.0/fonts/SBSansText/";
	...
	cl = function() {
		var args = [];
		for (var _i = 0; _i < arguments.length; _i++) args[_i] = arguments[_i];
		return args.join(" ").replace(/undefined|false|true/g, "").replace(/\s+/g, " ").trim();
	};
	render$1 = function(_a) {
		...
		effect(function() {
			var toggleInputType = function() {
				setState(function(state) {
					return { inputType: state.inputType === "voice-input" ? "text-input" : "voice-input" };
				});
			};
		...
		effect(function() {
			...
			var handleAction = function(action) {
				if (typeof action.text !== "undefined") sendText(action.text);
				else if (action.type === "deep_link") window.open(action.deep_link, "_blank");
				else if (action.type === "server_action") sendServerAction(action.server_action);
				else console.error("Unsupported action", action);
			};
			var createSuggestionHandler = function(suggestion) {
				return function() {
					var action = suggestion.action, actions = suggestion.actions;
					if (action) handleAction(action);
					if (actions) actions.forEach(handleAction);
				};
			};
	...
	nativePanel = createComponent({
		baseHTMLTemplate: template$1,
		createRefs: function(root) {
			return {
	...
	renderNativePanel = function(props) {
		if (!root) {
		...
		if (props.hideNativePanel) {
			nativePanel.unmount();
			root.remove();
		} else nativePanel.mount(root, props);
	};
	...
	render = function(_a) {
		...
		effect(function() {
			return setState(__assign({}, defaultState));
		}, []);
		effect(function() {
			var offs = [];
			var handleStart = function() {
				recorder === null || recorder === void 0 || recorder.start();
			...
			offs.push(function() {
				return refs.ButtonStart.removeEventListener("click", handleStart);
			});
			refs.ButtonStop.addEventListener("click", handleStop);
			offs.push(function() {
				return refs.ButtonStop.removeEventListener("click", handleStop);
			});
			return function() {
		...
		effect(function() {
			...
			var handleSave = function() {
				if (record) onSave(record);
			};
			var handleCopy = function() {
				console.log("record to copy", record);
				if (record) navigator.clipboard.writeText(JSON.stringify(record, null, 4));
			};
			refs.ButtonSave.addEventListener("click", handleSave);
			offs.push(function() {
				return refs.ButtonSave.removeEventListener("click", handleSave);
			});
			refs.ButtonCopy.addEventListener("click", handleCopy);
			offs.push(function() {
				return refs.ButtonCopy.removeEventListener("click", handleCopy);
			});
			return function() {
				return offs.forEach(function(off) {
					return off();
	...
	renderAssistantRecordPanel = function(recorder, saver) {
		var div = document.createElement("div");
		document.body.appendChild(div);
		recordPanel.mount(div, {
			recorder,
			onSave: saver.save
		});
	};
	createConsoleLogger = function(level) {
	...
	createLogCallbackRecorder = function(defaultActive) {
		if (defaultActive === void 0) defaultActive = true;
			...
			handler: prepareHandler(function(entry) {
				switch (entry.type) {
					case "incoming":
						updateRecord(function(record) {
							var _a;
							if ((_a = entry.message.systemMessage) === null || _a === void 0 ? void 0 : _a.data) record.entries.push({
								type: entry.type,
								message: {
						...
						updateRecord(function(record) {
							if (entry.message.text) record.entries.push({
								type: entry.type,
	...
	createRecordDownloader = function() {
		return { save: function(record) {
	...
	prefix = "recovery_";
	createRecoveryStateRepository = function() {
	...
	FEATURES = JSON.stringify({ appTypes: [
		"DIALOG",
		"WEB_APP",
		"CHAT_APP"
	] });
	...
	initializeNativeSDKEmulator = function(_a) {
		var _b;
		...
		var logger = (function() {
			var _a;
			if (enableRecord && (recordParams === null || recordParams === void 0 ? void 0 : recordParams.logger) == null) return recorder.handler;
		...
		var assistant = createAssistant$1({
			...
			device,
			legacyDevice,
			settings: __assign(__assign({}, settings), {
		...
		var emitOnData = function(command) {
			var _a;
			if (clientReady && assistantReady && ((_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onData)) window.AssistantClient.onData(command);
		};
		var fn = function() {
			return __awaiter(void 0, void 0, void 0, function() {
				var res, _i, _a, item;
				...
				return __generator(this, function(_f) {
					switch (_f.label) {
						case 0: return [4, assistant.start({
							initPhrase,
						...
						case 1:
							res = _f.sent();
							if (initPhrase && res) {
								if (((_b = res.app_info) === null || _b === void 0 ? void 0 : _b.frontendType) !== "WEB_APP") alert("Ваш навык не зарегистрирован, или указаны ошибочные данные. Следуйте инструкции: https://github.com/salute-developers/salutejs-client#аутентификация");
								...
								if (res === null || res === void 0 ? void 0 : res.feature_launcher) initialSmartAppData.push({
									type: "feature_launcher",
									feature_launcher: res.feature_launcher
								});
								for (_i = 0, _a = (res === null || res === void 0 ? void 0 : res.items) || []; _i < _a.length; _i++) {
									item = _a[_i];
									if (item.command != null && item.command.type === "smart_app_data") initialSmartAppData.push(__assign(__assign({}, item.command), { sdk_meta: { mid: "-1" } }));
								}
								window.appInitialData = initialSmartAppData;
								if (appInfo && appInfo.applicationId) {
									assistant.setActiveApp(appInfo, function() {
										var _a, _b;
										return Promise.resolve(((_b = (_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onRequestState) === null || _b === void 0 ? void 0 : _b.call(_a)) || {});
									});
								...
								if (clientReady && ((_c = window.AssistantClient) === null || _c === void 0 ? void 0 : _c.onData)) {
									initialSmartAppData.forEach(function(c) {
										var _a, _b;
										return (_b = (_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onData) === null || _b === void 0 ? void 0 : _b.call(_a, c);
									});
									!((_d = window.AssistantClient) === null || _d === void 0) && _d.onStart && ((_e = window.AssistantClient) === null || _e === void 0 || _e.onStart());
								}
								assistantReady = true;
							}
							return [2];
		...
		window.AssistantHost = {
			...
			close: function() {
				...
				if (appInfo && appInfo.applicationId) {
					...
					if ((_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onRequestRecoveryState) recoveryStateRepository.set(appInfo.applicationId, window.AssistantClient.onRequestRecoveryState());
				}
				assistant.closeApp();
			...
			ready: function() {
				var _a, _b, _c;
				if (assistantReady && ((_a = window.AssistantClient) === null || _a === void 0 ? void 0 : _a.onData)) !((_b = window.AssistantClient) === null || _b === void 0) && _b.onStart && ((_c = window.AssistantClient) === null || _c === void 0 || _c.onStart());
				clientReady = true;
			},
				...
				return __awaiter(this, void 0, void 0, function() {
					return __generator(this, function(_a) {
						switch (_a.label) {
							...
							case 1:
								_a.sent();
								assistant.sendServerAction(JSON.parse(payload), messageName || void 0);
								return [2];
						}
					});
				});
					...
					return __generator(this, function(_b) {
						switch (_b.label) {
							...
							case 1:
								...
								assistant.sendServerAction(data, messageName || "SERVER_ACTION", requestId, void 0, mode);
								return [2];
						}
					});
				});
		...
		var subscribeToListenerStatus = function(cb) {
			return assistant.on("assistant", function(_a) {
				...
				if (listener) cb(listener.status === "stopped" ? "stopped" : "listen");
			});
		};
		var subscribeToListenerHypotesis = function(cb) {
			return assistant.on("assistant", function(event) {
				if (event.asr) cb(event.asr.text, typeof event.asr.last === "undefined" ? false : event.asr.last);
			});
		};
		var updateDevUI = function(params) {
			...
			if (nativePanel) {
				...
				(render || renderNativePanel)(__assign(__assign(__assign({}, props), params), {
					sendText,
		...
		assistant.on("app", function(event) {
			...
			switch (event.type) {
				case "close":
					(_a = window.AssistantHost) === null || _a === void 0 || _a.close();
					break;
				case "command":
		...
		assistant.on("command", function(command) {
			if (command.type === "system" && command.system.command.toUpperCase() === "BACK") window.history.back();
		});
		assistant.on("vps", function(event) {
			...
			for (var _i = 0, _c = systemMessage.items || []; _i < _c.length; _i++) {
				var item = _c[_i];
				if (item.bubble) bubbleText = item.bubble.text;
			}
	...
	stand = "wss://nlp2b2c.devices.sberbank.ru/vps/";
	channelForSurface = {
		COMPANION: "COMPANION_B2C",
		SBOL: "SBOL"
	};
	createAssistantDev$1 = function(_a) {
		var getState = _a.getState, getRecoveryState = _a.getRecoveryState, ready = _a.ready, _b = _a.surface, surface = _b === void 0 ? "SBERBOX" : _b, userChannel = _a.userChannel;
		var nativePanel = initializeNativeSDKEmulator(__assign(__assign({}, __rest(_a, [
			"getState",
			"getRecoveryState",
			"ready",
			"surface",
		...
		return __assign(__assign({}, createAssistant({
			getState,
			getRecoveryState,
			ready
		})), { nativePanel });
	...
	createSmartappDebugger$1 = function(_a) {
		var token = _a.token, getState = _a.getState, getRecoveryState = _a.getRecoveryState, ready = _a.ready, _b = _a.settings, settings = _b === void 0 ? {} : _b, nativePanel = _a.nativePanel, sdkParams = __rest(_a, [
			"token",
			"getState",
			"getRecoveryState",
			"ready",
		...
		try {
			if (parseJwt(token).exp * 1e3 <= Date.now()) {
		...
		return createAssistantDev$1(__assign(__assign({}, sdkParams), {
			token,
...
//#endregion
//#region node_modules/@salutejs/client/esm/createAssistantDev.js
var createAssistantDev = (init_createAssistantDevOrigin(), __toCommonJS(createAssistantDevOrigin_exports)).createAssistantDev;
var createSmartappDebugger = (init_createAssistantDevOrigin(), __toCommonJS(createAssistantDevOrigin_exports)).createSmartappDebugger;
//#endregion
//#region node_modules/@salutejs/client/esm/index.js
init_createAssistant();
init_record_0905f37a();
init_assistant_c0a66ca1();
init_sdk_7c3cb15e();
init_client_ed11c12c();
init_voicePlayer_705b49ef();
var GetHistoryResponse = protoExports.GetHistoryResponse;
var InitialSettings = protoExports.InitialSettings;
var PacketWrapperFromServer = asrExports.PacketWrapperFromServer;
//#endregion
export { GetHistoryResponse, InitialSettings, MessageNames, PacketWrapperFromServer, VpsVersion, createAnswerFromMockByMessageGetter, createAssistant, createAssistant$1 as createAssistantClient, createAssistant$1 as createAssistantSDK, createAssistantDev, createAssistantHostMock, createAssistantHostMockWithRecord, createAudioRecorder, createClient, createMockRecorder, createMockWSCreator, createNavigatorAudioProvider, createOnlineRecordPlayer, createRecordOfflinePlayer as createRecordPlayer, createSmartappDebugger, createVoicePlayer };

//# sourceMappingURL=@salutejs_client.js.map